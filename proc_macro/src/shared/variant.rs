...
pub enum VarFields<Attr> {
    Named(Brace<InputPunctuated<VarFieldNamed<Attr>, Token![,]>>),
    Unnamed(Paren<InputPunctuated<VarFieldUnnamed<Attr>, Token![,]>>),
    Unit,
}
...
impl<T> VarFields<T> {
    pub fn len(&self) -> usize {
        match self {
            VarFields::Named(fields) => fields.inner.len(),
            VarFields::Unnamed(fields) => fields.inner.len(),
            VarFields::Unit => 0,
        }
    }
}
...
pub fn generics_needed_by_variant<T>(
    ...
    let (enum_lfs, enum_tys, enum_consts) = {
        ...
        for param in &enum_generics.input.params.inner {
            match param {
    ...
    _Some(SaneGenerics {
        input: InputGenerics {
            ...
            params:   InputPunctuated { inner: params },
