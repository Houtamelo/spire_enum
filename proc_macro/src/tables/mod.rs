...
fn parse_table_metas(input: TokenStream1) -> Result<SaneTableMetas> {
	let input_attrs = syn::parse::<Punctuated<Meta<TableMeta>, Token![,]>>(input)?;

	let mut sane = SaneTableMetas::default();
	let (syn_metas, custom_attrs) = split_input_metas(input_attrs);
	...
	for meta in custom_attrs {
		match meta {
			...
			TableMeta::Attrs(SettingAttrs { kw: _, attrs }) => {
				sane.syn_metas.extend(attrs.into_inner().into_iter())
			}
		}
	}
